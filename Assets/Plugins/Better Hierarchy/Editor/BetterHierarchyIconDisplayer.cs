...
namespace Utilities.BetterHierarchy
{
    ...
    public static class BetterHierarchyIconDisplayer
    {
        ...
        private static readonly HashSet<EntityId> additionalSelectedEntityIDs;
        private const float HIERARCHY_ICON_WIDTH = 18.5f;

        static BetterHierarchyIconDisplayer()
        {
			additionalSelectedEntityIDs = new HashSet<EntityId>();
        ...
        private static void OnEditorUpdate()
        {
			...
			additionalSelectedEntityIDs.Clear();
        }
        ...
        private static void OnHierarchyWindowItemOnGUI(EntityId entityID, Rect selectionRect)
        {
            ...
            HierarchyObjectStatus SetupHierarchyObjectStatus()
            {
                ...
                objectStatus.IsSelected = Selection.entityIds.Contains(entityID);
            ...
            void UpdateSelectedObjectsList(HierarchyObjectStatus objectStatus)
            {
                if (objectStatus.IsSelected || (objectStatus.IsDropDownHovered && MouseStatus.IsMouseDown))
                {
                    ...
                    additionalSelectedEntityIDs.Add(entityID);
                }
                else
                    additionalSelectedEntityIDs.Remove(entityID);
            }
        ...
        {
            int selectedAmount = Selection.entityIds.Length > 1 ? Selection.entityIds.Length : additionalSelectedEntityIDs.Count;
