...
namespace Assets._Project._Scripts.SerializeMethod.Editor
{
    // Single drawer handling both generic SerializedCallback<T> and non-generic SerializedCallback
    ...
    public class SerializedCallbackDrawerUI : PropertyDrawer
    {
        ...
        void ShowMethodDropdown(Object target, SerializedProperty methodProp, SerializedProperty property, Button methodButton, VisualElement root)
        {
            ...
            // - generic SerializedCallback<T> -> T
            // - non-generic SerializedCallback -> void
            Type callbackType = fieldInfo.FieldType;
            Type expectedReturn = typeof(void);
            if (callbackType.IsGenericType)
            {
                var args = callbackType.GetGenericArguments();
                if (args.Length > 0)
                    expectedReturn = args[0];
            }

            var methods = targetType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .Where(m => m.ReturnType == expectedReturn)
                .ToArray();
            ...
            for (int i = 0; i < parametersProp.arraySize; i++)
            {
                ...
                switch (paramType)
                {
                    ...
                    case ValueType.Vector3:
                        ...
                        {
                            value = vector3Prop.vector3Value
                        };
