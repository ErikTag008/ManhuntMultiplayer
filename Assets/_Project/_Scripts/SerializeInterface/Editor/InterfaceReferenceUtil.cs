using UnityEditor;
using UnityEngine;
namespace Assets._Project._Scripts.SerializeInterface.Editor
{
    public class InterfaceReferenceUtil
    {
        ...
        static void DrawInterfaceNameLabel(Rect position, string displayString, int controlID)
        {
            ...
            {
                const int ADDITIONAL_LEFT_WIDTH = 3;
                const int VERTICAL_INDENT = 1;

                var content = EditorGUIUtility.TrTextContent(displayString);
                var size = _labelStyle.CalcSize(content);
                var labelPos = position;

                labelPos.width = size.x + ADDITIONAL_LEFT_WIDTH;
                labelPos.x += position.width - labelPos.width - 18;
                labelPos.height -= VERTICAL_INDENT * 2;
                labelPos.y += VERTICAL_INDENT;
